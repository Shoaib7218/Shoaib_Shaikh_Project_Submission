...
CREATE OR REPLACE PACKAGE BODY task_search_pkg AS
    ...
    BEGIN
        ...
        --  Count query — wrapped title/description OR in parenthesess
        -- before: archived and status filter only applied to description matches
        -- After:  all three conditions apply to every row
        SELECT COUNT(*)
          INTO p_total_count
          FROM tasks
         WHERE archived = 0
        ...
        --  Paginated SELECT — same parentheses fix applied here too
        -- Before: same broken precedence as the COUNT query above
        -- After:  consistent logic between count and results
        OPEN p_results FOR
